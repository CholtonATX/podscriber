...
@click.option("--database", default=None, help="Override the Notion database ID from .env")
def main(episode: int | None, from_episode: int | None, limit: int | None, dry_run: bool, feed_url: str | None, database: str | None) -> None:
    """Podscriber: transcribe and extract brewing insights from podcast episodes."""
    try:
        config = load_config(feed_url_override=feed_url, database_id_override=database)
